...
- (void)commonInit {
    ...
    tagInputField_.backgroundColor = [UIColor groupTableViewBackgroundColor];
    tagInputField_.delegate = self;
    tagInputField_.font = [UIFont fontWithName:[MRemoteConfig secondaryFontName] size:14];
...
- (void)reloadTagSubviews {
    ...
    UIColor *tagBackgrounColor = _tagsBackgroundColor != nil ? _tagsBackgroundColor : [UIColor colorWithWhite:0.85 alpha:1];
...
-(void)textFieldDidBeginEditing:(UITextField *)textField{
    if ([self.tapDelegate respondsToSelector:@selector(tagsControlDidBeginEditing:)]) {
        [self.tapDelegate tagsControlDidBeginEditing:self];
    }
}
