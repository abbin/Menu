...
- (void)yms_presentCustomAlbumPhotoView:(UINavigationController *)pickerViewController delegate:(id<YMSPhotoPickerViewControllerDelegate>)delegate
{
    PHAuthorizationStatus status = [PHPhotoLibrary authorizationStatus];

    if (status == PHAuthorizationStatusAuthorized) {
        YMSPhotoPickerViewController *vc = pickerViewController.viewControllers[0];
